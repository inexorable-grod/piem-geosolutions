      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <h2 className="font-display text-3xl sm:text-4xl lg:text-5xl font-bold text-[var(--text)] mb-4">
            Nuestro <span className="text-gradient-orange">Equipo</span>
          </h2>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {TEAM_DEPARTMENTS.map((dept) => (
            <div
              key={dept.id}
              className="group p-6 rounded-2xl border border-[var(--border)] bg-[var(--card)] hover:border-orange-400/40 hover:shadow-orange transition-all duration-300"
            >
              <div
                className="w-2 h-8 rounded-full mb-4"
                style={{ backgroundColor: dept.color }}
              />
              <h3 className="font-display text-base font-semibold text-[var(--text)] mb-3 group-hover:text-orange-400 transition-colors">
                {dept.name}
              </h3>
